...
**🔴 Немає апаратного GPU — передавайте `renderer: 'canvas'` явно. Не покладайтесь на `'auto'`.**

Це не обережність, а вимір. Той самий датасет (`Staff · 1M`), той самий жест панорамування,
...
`'auto'` обрало гілку **вшестеро повільнішу** саме там, де фолбек і мав спрацювати. Причина —
нижче: це підказка браузеру, а не перевірка.

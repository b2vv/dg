...
⚠️ **SVG-експорт завжди малює button-group.** Якщо у вас `cell-flood`, SVG відрізнятиметься від
екрана — SDK скаже про це (див. §10). PNG/PDF знімаються з живого канвасу й вірні завжди.
...
- Розходження рушіїв контуру повідомляється:

```ts
await diagram.export({ format: 'svg', onDiagnostic: (m) => console.warn(m) });
// без onDiagnostic те саме йде в console.warn — мовчки воно не розійдеться
```
...
Він присутній **завжди**, тож список більше не буває порожнім після монтування. Якщо ви показуєте
діагностику як попередження, фільтруйте його за префіксом `'Renderer: '` — префікс стабільний
і є частиною контракту.
...
- **`cell-flood` не потрапляє в SVG** (§6, §10).

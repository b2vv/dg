...
1. **Явний вибір хоста** сильніший за все — якщо ви передали `'canvas'` або `'webgl'`, нічого
   не питається.
2. **Ім'я рушія.** SDK бере тимчасовий WebGL-контекст, читає `UNMASKED_RENDERER_WEBGL` і
   відпускає контекст. Якщо ім'я містить відомий програмний растеризатор — `SwiftShader`,

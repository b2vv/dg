 ...
 * @param {number | null} [node_width]
 * @param {number | null} [node_height]
 * @param {number | null} [h_gap]
 * @param {number | null} [v_gap]
 * @param {number | null} [margin]
 * @returns {any}
 */
export function computeOrgRowTreeLayout(organizations, expanded_root_id, direction, node_width, node_height, h_gap, v_gap, margin) {
    const ptr0 = passStringToWasm0(expanded_root_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    var ptr1 = isLikeNone(direction) ? 0 : passStringToWasm0(direction, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    var len1 = WASM_VECTOR_LEN;
    const ret = wasm.computeOrgRowTreeLayout(organizations, ptr0, len0, ptr1, len1, !isLikeNone(node_width), isLikeNone(node_width) ? 0 : node_width, !isLikeNone(node_height), isLikeNone(node_height) ? 0 : node_height, !isLikeNone(h_gap), isLikeNone(h_gap) ? 0 : h_gap, !isLikeNone(v_gap), isLikeNone(v_gap) ? 0 : v_gap, !isLikeNone(margin), isLikeNone(margin) ? 0 : margin);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./org_hierarchy_core_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('org_hierarchy_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();

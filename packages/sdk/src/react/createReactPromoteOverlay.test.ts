  ...
  it('success: the layer carries the camera while the cards stay put', async () => {
    ...
    await act(async () => {
      overlay = createReactPromoteOverlay({
        ...
        component: DefaultPromoteCard,
        mode: 'near-visible',
        // 🔴 **Довгий settle навмисно, і це не «щоб не флейкало».** Стан, який
        // перевіряють асерти нижче, за побудовою **тимчасовий**: він живе рівно
        // до `settleMs`, після чого `sync()` скидає трансформ у `''`. Зі
        // спільним `settleMs = 15` тест вимагав, щоб три рядки нижче виконались
        // за 15 мс **стінного** часу — і під паралельною сюїтою вони не
        // встигали: падало `expected '' to be 'translate(40px, -10px) scale(1)'`,
        // тобто осідання встигало раніше за асерт.
        //
        // Чекати тут нема на що: предмет — саме **невідкладений** запис. Тому
        // дедлайн прибрано з рівняння, а не збільшено «з запасом». Осідання
        // перевіряє наступний тест, і там `settleMs` малий — бо там воно й є
        // предметом.
        settleMs: 10_000,
      });
    });

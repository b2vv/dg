...
export interface TestAnchorCandidate {
  ...
  /**
   * Бокс кнопки розгортання, у тому самому світовому просторі, що й
   * {@link TestAnchorCandidate.world} — тобто **до** камери (T115 крок 2).
   *
   * Відсутній, коли кнопки в цьому кадрі немає, і причин тому кілька: вузол —
   * лист, камера на далекому LOD, вузол промотований у HTML, або chrome
   * вимкнено конфігом. Саме тому поруч їде {@link hasChildren}: без нього
   * «бокса немає» не відрізнити від «дітей немає».
   */
  expander?: { x: number; y: number; width: number; height: number };
  /**
   * Чи має вузол що розгортати — властивість **моделі**, від камери незалежна.
   *
   * Визначена лише для `kind === 'organization'`: у персони питання не має
   * відповіді, у посади воно означає інше (адмінпідлеглі). Для решти видів
   * поле **відсутнє**, і це видно з `kind`, а не вгадується.
   */
  hasChildren?: boolean;
}

...
## 8. Виправлення (2026-09-02, друга сесія)

| Знахідка | Стан | Коміт |
|---|---|---|
| `getData()` віддає живий стан | ✅ **закрито** — глибока копія | `ec1e75a` |
| `isDiagramData` пропускає хибні форми | ✅ **закрито** частково — колекції мусять бути масивами | `ec1e75a` |
| bare-grid без перевірки епохи | ✅ **закрито** | `ec1e75a` |
| Rust без гейтів | ✅ **закрито** — `fmt --check` + `clippy --all-targets -D warnings` у CI | `21d3560` |
| Критично: глибина row-tree | 🔵 відкрито — див. §7.2–7.4 |  |
| `setData` без епох | 🔵 відкрито |  |
| Мутації не одна транзакція | 🔵 відкрито |  |
| Кореневий барель і тест-хуки | 🔵 відкрито — **зміна публічного API**, поріг маніфесту спрацьовує |  |
| Два рушії контуру | 🔵 продуктова розвилка (`work/AGENDA.md` Р1) |  |

### Що зроблено інакше, ніж радив звіт

**`getData`.** Порада була «deeply readonly view (and freeze in development) or an explicit
snapshot/clone». Readonly-тип — це зміна **публічного API**, тобто повний цикл за маніфестом;
freeze живого об'єкта зламав би внутрішні мутації, бо це той самий об'єкт. Тому — копія.
**Глибока, а не поверхнева**, і це вимір, а не смак: поверхнева коштує 0,01 мс проти 7,6 мс на
4 000 посадах, але зупиняє лише `push`/`splice`, **не** `collapsed` чи координати — саме те, що
звіт називає прикладом. Напівзахист тут гірший за відсутність.

**Побічний ефект, вартий згадки:** три тести доводили «дані не змінились» через **ідентичність
посилання**. Цей інструмент існував лише тому, що акцесор віддавав внутрішній об'єкт, — він
зник разом із дефектом. Замінено на порівняння вмісту.

**`isDiagramData`.** Закрито лише **половину**: колекції тепер мусять бути масивами, тож
`positions: null` більше не проходить. Друга половина поради — «віддавати перевагу
`mappers.toDiagram`, коли його явно передали» — **не** зроблена свідомо: це змінює задокументовану
поведінку для хоста, який передає і дані, і мапер. Записано в коді як обмеження, а не приховано.

**Clippy.** Із шести знахідок виправлено три; дві лишились дозволеними **на місці, з причиною**
(вісім аргументів — це і є контракт TS↔WASM; десять — рекурсія, яку однаково переписуватиме
робота над глибиною), одна — дозволена на рівні тестового модуля. Не глушити конфігом на весь
крейт — те саме правило, що й у T85.

### Що звіт недооцінив

§7.2–7.4: критична межа не на 20 000, а **між 2 500 і 3 000**, вона про **глибину, а не обсяг**,
і меж **дві** — стек JS і стек WASM. Це змінює і формулювання задачі, і форму приймального тесту.

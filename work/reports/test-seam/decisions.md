# T115 — два рішення, що лишились: якір експандера і гейт

**Дата:** 2026-09-08 · **Базис:** `main` @ `95ec90e` · **Вимір:**
[`measurement.md`](./measurement.md) · **Задача:**
[T115](../../tasks/T115-test-seam-parity-with-host.md)

Кроки 1 і 3 закриті кодом. Ці два — ні, і не тому, що на них не вистачило часу: **обидва
додають або фіксують публічний контракт**, тобто це рішення, а не виконання.

---

## Як шов виглядає сьогодні

```mermaid
flowchart TB
    subgraph tests["e2e хоста / демо"]
        T1["page.getByTestId('node-ceo').click()"]
        T2["expandOrg(page, 'org-1')<br/>через window.__demoE2e"]
    end

    subgraph dom["DOM поверх канви"]
        A["createTestAnchorOverlay<br/>button[data-testid='node-&lt;id&gt;']<br/>CSS-пікселі, ресинк на кожен рух камери"]
    end

    subgraph sdk["Фасад OrgHierarchyDiagram"]
        L["listTestAnchors() → world-бокс картки"]
        W["worldBoxToScreen + getViewport + getCanvas"]
        E["expandOrg / focusByTestId"]
    end

    subgraph render["Рендер (Pixi)"]
        S["scene.listBoxes()<br/>бокс картки"]
        C["mountOrgNodeChrome<br/>кнопка експандера"]
    end

    T1 --> A --> L --> S
    A --> W
    T2 -.->|"обхід: міст, бо якоря немає"| E
    C -. "результат відкидається<br/>OrganizationNode.ts:254" .-> X(["немає боксу<br/>експандера"])
    X -.-> T2

    classDef gap fill:#fee,stroke:#c00,stroke-width:2px,color:#900
    class X,T2 gap
```

Суцільні стрілки — шлях, який працює. Пунктир — те, чим його обходять, бо однієї коробки бракує.

🔑 **Це не гіпотеза про хост — це наш власний код.** `e2e/flat-orgs.spec.ts:72` клікає вузол через
DOM-якір і працює. А розгортання в тому ж файлі йде через `expandOrg(page, 'org-1')`
(`e2e/demoBridge.ts:5-9`), тобто через `window.__demoE2e`. **Міст, який T115 хоче прибрати в
хоста, живий у нас — рівно в тій точці, де якоря немає.** Дірка не теоретична; ми вже платимо за
неї щодня.

---

## Рішення 1 — чи давати якір експандера

### Чому ззовні його не обчислити

Кут кнопки залежить від режиму сцени, і режимів більше, ніж здається:

| Умова | Де кнопка | Джерело |
|---|---|---|
| `gojsTree` | центр `(w − 13, h − 13)`, d = 26 — **низ-право** | `render/orgNodeChrome.ts:42-48` |
| дерево, звичайне | `x = w − 26, y = 4` — **верх-право** | `:95-106` |
| штат (chevron ▲/▼) | той самий верхньо-правий слот | `:107-117` |

Перемикач — не одна опція, а вираз:
`gojsTree = isGojsVertical(style) && style.gojsTreeExpander !== false`
(`render/OrganizationNode.ts:248`).

І два випадки, коли кнопки **немає взагалі**:

- `lod === 'far'` → chrome не монтується зовсім (`OrganizationNode.ts:246`);
- `chrome.kind === 'staff-expand' && gojsTree` → ранній вихід (`:251-253`).

**Це і є головний аргумент за необов'язкове поле:** експандер — не властивість вузла, а
властивість вузла **в поточному кадрі**. Обов'язкове поле брехало б у двох режимах із чотирьох.

### 🔴 Правило існування кнопки, і чому воно ламає наївне «необов'язкове поле»

Уточнення від користувача 2026-09-10: **кнопка є тоді й лише тоді, коли є діти.** Звірено з
кодом — правило реалізоване, але **не всюди**:

| Chrome | Правило `hasChildren` | Джерело |
|---|---|---|
| дерево (обидва варіанти) | ✅ **точно** — при `hasChildren === false` chrome не створюється **взагалі**, картка отримує лише кнопку меню | `DiagramRenderer.ts:1118-1127`, `OrganizationNode.ts:250-266` |
| штат (chevron ▲/▼) | ❌ **не реалізоване** — chevron монтується щоразу, коли хост дав `onStaffOrgExpandToggle` | `DiagramRenderer.ts:1145-1156` |

Асиметрія видна вже на рівні типів: `OrgTreeChrome` несе `hasChildren`, а `OrgStaffExpandChrome`
має лише `expanded` і `onToggle` (`orgNodeChrome.ts:8-22`) — тобто в нього просто **немає** чим
ухвалити це рішення. Наслідок: організація без жодної посади у штатній сцені все одно показує
chevron, який розгортає порожнечу. Це кандидат у дефекти окремо від T115.

#### Чому це міняє форму поля

Мій попередній аргумент за необов'язкове поле був «кнопки може не бути». Правило користувача
робить цю відсутність **значущою**: `expander === undefined` тепер читається як «вузол — лист».
І саме тому наївне `expander?: WorldBox` **не годиться**: воно перевантажене й означає
щонайменше три різні речі:

1. **дітей немає** — властивість моделі, стабільна;
2. **`lod === 'far'`** — chrome не змонтований зовсім (`OrganizationNode.ts:246`), властивість
   кадру, залежить від зуму;
3. **штат під `gojsTree`** — ранній вихід (`:251-253`).

🔴 **Практична ціна цієї двозначності — тест, зелений із хибної причини.** Перевірка «у листа
немає експандера» пройде й тоді, коли камера просто від'їхала далеко. Це рівно той клас, який у
цьому репо вже коштував і хибної задачі, і хибної спеки.

**Отже форма A уточнюється:** поряд із боксом потрібна ознака, що розрізняє модель і кадр —
найдешевше `hasChildren: boolean` у `TestAnchorCandidate`. Тоді `expander === undefined &&
hasChildren === true` означає «кнопка мала б бути, але кадр її не показує», а
`hasChildren === false` — «і не повинно бути». Без цього хост не може написати негативний тест,
який не бреше, а негативні тести — саме те, заради чого якір і потрібен.

✅ **Асиметрію закрито 2026-09-10 (реліз `0.4.1`), і це був дефект, а не стиль.**
`OrgStaffExpandChrome` дістав поле `hasStaff`, chevron монтується лише за ним, а джерело правди
взяте те саме, з якого будується розгорнутий блок: `StaffOrgCard.positionCount`
(`canvasLayout.ts:137`). Тобто поле, потрібне для `hasChildren` у якорі, тепер **існує в рендері**
— лишається протягти його до `TestAnchorCandidate`, а не вигадувати заново.

### Що доведеться зробити в коді

`mountOrgNodeChrome` **уже повертає** `{ expandButton?: Container }` — і цей результат
**відкидається** на `OrganizationNode.ts:254`. Тобто дані існують і викидаються за рядок до того,
як стали б корисні. Робота: утримати їх, підняти в `scene.listBoxes()` поруч із боксом картки,
і додати поле в `TestAnchorCandidate`.

### Варіанти

| # | Форма | Ламальність | Що дає | Що коштує |
|---|---|---|---|---|
| **A** | `expander?: WorldBox` у `TestAnchorCandidate` + другий якір `node-<id>-expander` в оверлеї | **не ламальна** (поле необов'язкове) | і DOM-шлях, і піксельний; чесно відсутнє там, де кнопки немає | утримати мітку в рендері, протягти крізь сцену |
| B | те саме, але поле обов'язкове | **ламальна** | тип «завжди є» простіший | брехня в двох режимах — доведеться підставляти бокс картки |
| C | окремий метод `listExpanderAnchors()` | не ламальна | не чіпає наявний тип | другий обхід сцени й другий список, який може розійтись із першим |
| D | нічого не робити | — | нуль коду | хост лишається на мості **і ми теж** — дірка вже виміряна в нашому репо |

### Рекомендація — **A**. Питання до хоста закрите **виміром хоста**

A виграє тому, що необов'язковість тут не поступка сумісності, а **точне відображення
реальності**: кнопки справді може не бути.

#### ✅ 2026-09-10 — хост відповів, і відповідь фальсифікована

Питання стояло так: їм потрібно **клікнути по експандеру** чи **перевірити, що розгортання
сталось**? Відповідь від сесії, що веде e2e в `cassiopeia-admin-ui`: **потрібен саме клік**, і
під це є два наявні тести:

| Тест | Що доводить |
|---|---|
| `tests/organization-hierarchy-diagram.e2e.ts:312` | клік по експандеру перемикає дітей **без вибору вузла** |
| `tests/organization-hierarchy-diagram.e2e.ts:328` | розгортання/згортання **ніколи не відкриває сайдбар** |

Обидва йдуть через `expanderCenter(key)` (`:172-179`) — місток `window.__orgHierarchy.expanderCenter`
— і `page.mouse.click`.

🔑 **Аргумент хоста сильніший за той, яким я обґрунтовував A, і його треба записати саме в
їхньому формулюванні:** обидва тести доводять **негативне** — що клік по кнопці **не** виділяє
вузол і **не** відкриває сайдбар. Виклик API розгортає сцену, **обходячи рівно той шар**, у якому
живе можлива вада: попадання в кнопку проти попадання в картку. Проба, яка розгортає програмно,
лишається зеленою навіть на коді, де **кнопка перекрита карткою**.

Тобто варіант D («нічого не робити») коштує хосту не зручності, а **єдиного гейта** на цю
поведінку — двох тестів, які нічим іншим не виражаються.

⚠️ **Межа цієї відповіді, названа самим хостом:** це вимір **їхніх тестів**, а не рішення про
міграцію. Чи переходить хост на SDK і в якому обсязі — питання замовника, не інженерів з жодного
боку. Отже закрито саме «чи потрібен якір експандера» (потрібен), і **не** закрито «робимо це
зараз» — обсяг лишається рішенням користувача.

---

## Рішення 2 — гейт: хто вимикає шов у проді

### Факти

| Хто | Чим гейтить | Джерело |
|---|---|---|
| хост | `import.meta.env.DEV`, у проді моста немає | T115 §«Що сказав хост» |
| наше **демо** | `if (!this.e2eMode) return;` — оверлей монтується лише при `?e2e=1` | `packages/demo/src/app/App.ts:600-605` |
| наш **SDK** | **нічим** — оверлей вмикає той, хто його викликав | `react/createTestAnchorOverlay.ts` |

### 🔴 Реальна небезпека, а не теоретична

При `interactive: true` якорі отримують `pointerEvents: 'auto'` і `opacity: 0.001`
(`createTestAnchorOverlay.ts:78-80`). Це **невидимі клікабельні кнопки поверх карток**. Лишити
таке в проді — віддати кліки користувача оверлею замість канви. При `interactive: false`
(типове) вони `pointerEvents: 'none'` і нікому не заважають — але тоді `.click()` б'є вже в
канву, і Playwright сам вирішує, чи це його влаштовує.

Тобто небезпечний не оверлей, а **саме `interactive: true`**, і це рівно той аргумент, який має
стояти в доці великими літерами.

### Варіанти

| # | Хто гейтить | Оцінка |
|---|---|---|
| **1** | **хост, а SDK це прямо документує** | 🟢 SDK не знає бандлера хоста; читати `import.meta.env` у бібліотеці = зашити припущення про збірку. Оверлей інертний, поки його не змонтували, тож дефолт уже безпечний |
| 2 | SDK читає `import.meta.env.DEV` сам | 🔴 ламає всіх, хто не на Vite; у Rsbuild-демо це інша змінна. Бібліотека почне залежати від тулчейна споживача |
| 3 | окремий підшлях `@org-hierarchy/sdk/testing`, який tree-shake викидає з прод-бандла | 🟡 найчесніше технічно, але це нова точка входу в `package.json#exports` — тобто знову рішення про публічну поверхню, і більше роботи, ніж вартує |

### Рекомендація — **1**, з двома конкретними діями

1. Дописати в §14 `USAGE.md` абзац: `interactive: true` — **тільки під гейтом середовища**, з
   поясненням про невидимі кнопки поверх канви. Зараз доки про це не каже, а це найдорожча
   помилка, яку споживач може зробити з цим API.
2. Назвати **наше демо** зразком гейта (`?e2e=1` → `e2eMode`) — у нас уже є працююча відповідь,
   просто ніде не сказано, що вона зразкова.

Це не потребує ані нового коду в SDK, ані нової точки входу — лише зафіксувати контракт, який
де-факто вже діє.

---

## Підсумок для рішення

| Пункт | Що вирішити | Моя рекомендація |
|---|---|---|
| Якір експандера | давати чи ні, і в якій формі | **A** — необов'язкове поле + другий якір. Питання до хоста **закрите його ж виміром** (2026-09-10): клік потрібен, два тести падають без нього. Лишилось рішення про **обсяг і термін**, не про потребу |
| Гейт | хто вимикає в проді | **1** — гейтить хост, SDK документує; додати попередження про `interactive: true` і назвати демо зразком |

Обидва пункти можна робити незалежно. Другий — дешевий і без коду; перший — торкається рендера,
сцени й публічного типу, тож іде через `spec-flow`.

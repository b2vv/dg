...
**Дата:** 2026-09-07 (другий перезбір цього дня)
**Базис:** `main` @ `fbb817b` (після T113; попередній базис `0afa280` відстав на 19 комітів)
...
**Стан:** живого P0 немає. За 2026-09-06/07 закрито **T111**, **T117 Гап 1**, **T110** (+ реліз `0.3.0`) і **T113**; заведено T115, T116, T117. Агент-реді лишились: **T80** (прибрати `cell-flood` — найбільший обсяг, розвилку закрито ще 2026-09-06), T101, T102 блок Б, T106, T109, T114, T115. Заблоковані продуктовим рішенням: **T116** (сім розвилок), **T117 Гап 2**, T112, T56, T61, T67 Phase 2.
...
У SDK **144 модулі + 127 тестових файлів**; зелена база — **997 sdk + 108 demo** unit (заміряно 2026-09-07 після T113), **73 e2e**, **40** Rust і окремо **19** тестів на самі скрипти гейта (`npm run test:scripts`).
...
9. **T101 / T106 / T109 / T110** — агент-реді, нічого не блокує. **T105 закритий 2026-09-06** (PR #82): повний цикл `spec-flow` без пропущених гейтів — `plan-critique` 4/4, `plan-defense` 3 дірки, `code-review` 2/2. ⚠️ **T101 більше не та задача, що написана:** його гіпотеза («винні паралельні воркери») **спростована 2026-09-04** — падає 2 з 3 при `--workers=1`. T105 вище порога пайплайна (публічний API) — `spec-flow`. T103/T104/T108 закриті.
10. **15 публічних методів поза `docs/USAGE.md`** — названо, не закрито; baseline у `scripts/check-docs.mjs` (21 → 16 → 15).
...
12. 🔴 **Host: прибрати GoJS — і це досі пункт нуль, а не дванадцять.** Хост SDK не імпортує (факт №5). **T117 Гап 1 закрито** (`68854d7`). Лишились: **T116** (org-level reparent) — spec+plan написані, коду немає, **сім продуктових розвилок** чекають людини, і `plan-critique` по цій парі **не проганялась**; **T117 Гап 2** (move cross-org / merge / clone) — заблокований питанням, чи це взагалі відповідальність SDK; **T115** (дев-міст `window.__orgHierarchy`) — хост називає це найнедооціненішою статтею витрат міграції.

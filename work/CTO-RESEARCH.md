...
**Дата:** 2026-09-03
**Базис:** `origin/main` @ `ece03e4` (після T101–T107, docs freshness gate, Rust CI baseline)
...
**Стан:** живого P0 немає. Від попереднього базису закрито T102 блок А (row-tree більше не вбиває WASM, підтримана глибина 2 500) і T107 (контурний рендер прискорено в TS, WASM-порт відхилено вимірюванням). Черга далі — **суміш**: агент-реді структурний борг (T101, T102 блок Б, T103–T106) і продуктові рішення (T80 — який рушій контуру лишається; T56 — звірений із кодом сьогодні, чекає вибору продукту; T92 — заблоковано на числі з заліза замовника).
...
7. **T98** — `renderer: 'auto'` у цьому середовищі **не** перейшло на canvas: `failIfMajorPerformanceCaveat` — підказка браузеру, а не перевірка. Хост, який нічого не передав, їде на 8 fps поруч із невикористаним шляхом на 60. Потрібне продуктове рішення, не лише код.

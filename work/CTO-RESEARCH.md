...
**Дата:** 2026-09-06
**Базис:** `main` @ `e411252` (після T105; попередній базис `9f0c7f3` відстав на 5 комітів)
...
**Стан:** живого P0 немає. За 2026-09-04/05 закрито **п'ять** задач — T92, T98, T104, T103, T108 — і заведено T109. Лишились агент-реді T101, T102 блок Б, T105, T106, T109 і продуктові рішення (T80, T56, T61, T67 Phase 2).
...
**Новий CI-гейт — `Docs freshness`** (`npm run check:docs`, [`scripts/check-docs.mjs`](../scripts/check-docs.mjs), доданий `ece03e4`). Ловить **чотири** речі: (1) биті відносні `.md`-лінки — архівація 2026-09-02 лишила 25 таких; (2) публічний метод `OrgHierarchyDiagram` поза `docs/USAGE.md` — поріг пайплайна визначає публічний API саме через цей файл, тож метод поза ним для нього не існує; **15** методів зафіксовано як борг-baseline у `scripts/check-docs.mjs` (21 → 16 з T104, → 15 з T105; може лише зменшуватись); (3) **експорт кореневого барелю на `*ForTests`** — скан у [`scripts/barrelSurface.mjs`](../scripts/barrelSurface.mjs) читає **текст** `index.ts`, бо рантайм-неймспейс не побачив би `export type { FooForTests }`; це **єдина частина гейта з власними тестами** (`npm run test:scripts`, stdlib `node --test`, вісім штук, та сама CI-джоба) — решта перевірок гейта тестів не має; (4) цей файл (`work/CTO-RESEARCH.md`) не старший за `HEAD` більш ніж на **25 комітів** — межа з шапки цього самого файлу, перевірена `git rev-list --count`. Чого гейт **не** ловить — чи твердження в доці досі правдиве: `T56` десять днів заявляв контур на WASM після того, як WASM звідти прибрали, і жоден лінк-чекер цього не бачить.

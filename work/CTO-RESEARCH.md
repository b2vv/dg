...
**Дата:** 2026-09-10
**Базис:** `main` @ `772fa3f` (після мержу T109; попередній базис `6b199a4` відстав на 29 комітів
...
**Стан:** живого P0 немає. За 2026-09-06/08 закрито **T111**, **T117 Гап 1**, **T110** (+ реліз `0.3.0`), **T113**, **T114**, **T118**, **T101**, **T106**, **T119 крок 0** і **T115 кроки 1–3**; заведено T115–T119. ✅ **T80, T102 блок Б і T109 закриті.** Агент-реді пунктів у черзі **не лишилось**: усе, що є, впирається в рішення. Заведено **T120** (стеля глибини — вимір у браузері плюс продуктове рішення про `tidy_tree`) і **T121** (`setStaffFocus` — намір чи стан?). Єдине, що рухається просто зараз, — **T115 крок 2**: потреба доведена виміром хоста, спека пройшла GATE 1 із трьома блокуючими. Заблоковані продуктовим рішенням: **T116** (сім розвилок), **T117 Гап 2**, **T115 кроки 2 і 4**, **T119** (саме рознесення фасаду), T112, T56, T61, T67 Phase 2.
...
3. **Якоря експандера немає, і хост його не обчислить.** Кут кнопки залежить від режиму сцени, а в двох режимах кнопки немає взагалі ([`render/orgNodeChrome.ts:42-48`](../packages/sdk/src/render/orgNodeChrome.ts) проти `:95-117`; `OrganizationNode.ts:246,248,251-253`). **І ми платимо за це самі:** розгортання в наших e2e йде через міст `window.__demoE2e` (`e2e/demoBridge.ts:5-9`) — тобто рівно той обхід, який T115 хоче прибрати в хоста. Розбір і чотири варіанти — [`reports/test-seam/decisions.md`](./reports/test-seam/decisions.md).
...
| 12 | **Юніт-перф-тести чутливі до паралельного навантаження.** `paintMagneticGroupsCost` падає під навантаженням і проходить без нього. ✅ **T101 закрито 2026-09-08** — і закрито не фіксом тесту, а виміром: це **клас тестів**, а не тест, тож ретраї з CI прибрані (вони б заховали T118). Сам ризик лишається чинним для юнітів | [T101](./tasks/T101-e2e-flakes-only-local.md), [звіт](./reports/auto-software-fallback/report.md) §4 |
...
| 28 | **Наші власні e2e тримають той самий міст, який T115 хоче прибрати в хоста.** Клік по вузлу йде через DOM-якір, а розгортання — через `window.__demoE2e` (`e2e/demoBridge.ts:5-9`), бо якоря експандера немає. Тобто дірка не гіпотетична: ми платимо за неї у власній сюїті | [`reports/test-seam/decisions.md`](./reports/test-seam/decisions.md) |

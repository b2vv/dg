...
**Дата:** 2026-09-07
**Базис:** `main` @ `0afa280` (після T111 і читання хоста; попередній базис `e411252` відстав на 16 комітів)
...
**Стан:** живого P0 немає. За 2026-09-06/07 закрито **T111** (сім кроків, змержено `31b98b3`, запушено) і заведено **T115, T116, T117**. Агент-реді: **T117 Гап 1** (root-guard, TDD-таблиця вже в задачі — найдешевший наступний крок), T101, T102 блок Б, T106, T109, T110, T113. Заблоковані продуктовим рішенням: **T116** (сім розвилок у [`spec.md`](./reports/org-reparent/spec.md)), **T117 Гап 2**, T80, T56, T61, T67 Phase 2.
...
У SDK **143 модулі + 126 тестових файлів**; зелена база — **941 sdk + 108 demo** unit (заміряно 2026-09-07), **73 e2e**, і окремо **13** тестів на самі скрипти гейта (`npm run test:scripts`).
...
| 20 | ⚠️ **Два сусідні reparent-мутатори тепер поводяться по-різному.** T111 прибрав мовчазну відмову в `movePersonToCell`, але `reparentPosition` (`OrgHierarchyDiagram.ts:1671-1673`) досі гасить `InteractionError` мовчки й **навіть без `render()`** — тобто кадр лишається брехати | розвилка №6 у [`reports/org-reparent/spec.md`](./reports/org-reparent/spec.md) |
...
12. 🔴 **Host: прибрати GoJS — і це досі пункт нуль, а не дванадцять.** Хост SDK не імпортує (факт №5). Із трьох задач, заведених читанням: **T117 Гап 1** (root-guard для `reparentPosition`) — агент-реді, TDD-таблиця вже в задачі, найдешевший наступний крок; ⚠️ **не** зашити заборону надто широко — корінь **не можна рухати**, але робити корінь чиїмось керівником нормально, і тест на це обов'язковий. **T116** (org-level reparent) — spec+plan написані, коду немає, **сім продуктових розвилок** чекають рішення людини (`reports/org-reparent/spec.md` §Відкриті питання), і `plan-critique` по цій парі **не проганялась**. **T117 Гап 2** (move cross-org / merge / clone) — заблокований продуктовим рішенням: чи це взагалі відповідальність SDK.
...
21. **Питаєш «а як це в хості?» — читай хостовий код, не переказ.** Одне таке читання

...
10a. 🔴 **`CHANGELOG.md` мертвий від `0.2.0` (26 серпня)** — відтоді `docs/USAGE.md` змінили **десять** комітів, серед них новий публічний метод `searchAll`, конфіг `initialExpand`, канал `RenderFailure` і **змінений контракт `setData`**. Хост, що читає CHANGELOG перед оновленням, не дізнається про них нічого. [T110](./tasks/T110-changelog-died-at-0.2.0.md).

...
🔴 **Виправлено 2026-09-06 (рішення продукту).** Тут стояло «Matrix — коли **всі org** згорнуті».
Це записано **невірно**: правило **локальне, а не глобальне** — матрицею лягає будь-який
**набір братів** (діти одного батька), **усі члени якого згорнуті**. Стара редакція була **окремим випадком** нової: коли згорнуто
геть усе, верхній рівень братів теж увесь згорнутий, тож обидва формулювання давали однакову
картинку — і саме тому помилку не ловили. Розходяться вони там, де вузол розгорнутий, а його діти
ні. **Код досі реалізує стару редакцію** (`layout/orgMode.ts:9-11`) — див.
[T113](archive/tasks-2026-09-11.md).

```mermaid
stateDiagram-v2
  [*] --> Matrix: набір братів, де всі члени collapsed
  Matrix --> RowTree: expand ≥1 у цьому наборі
  RowTree --> Matrix: collapse всі в наборі
```

| Розкладка набору братів | Умова | Layout |
|-------|-------|--------|
| **Matrix** | **усі** брати в наборі `collapsed: true` | Sparse adjacency; порядок змінюється D&D |
| **Row-tree** | ≥1 брат у наборі expanded | Рядки за depth: row 1, 2, 3… |

**Правило рекурсивне:** застосовується до кожного набору братів окремо, а не один раз до всієї
діаграми. Верхній рівень — такий самий набір, тож «усі org згорнуті → matrix» лишається чинним як
наслідок, а не як означення.
...
> [T80](archive/tasks-2026-09-11.md).
>

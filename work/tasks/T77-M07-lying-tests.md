...
| `incremental.test.ts` | fingerprint без foreign cells | ✅ neighbor fingerprint + tests |
| `worker-bridge` echo | mock echo path |
| `pngExport` jsdom sniff | прод-код нюхає test runner |

## Acceptance

- [x] Кожен рядок вище або виправлений асерт, або позначений `it.fails` / видалений з green path.
- [x] Немає `navigator.userAgent.includes('jsdom')` у прод-експорті (inject seam / DI).

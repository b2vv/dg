...
| `npm run check:docs` | **свіжість доки, перед пушем** (джоба CI `Docs freshness`): биті `.md`-лінки · публічний метод `OrgHierarchyDiagram` поза `docs/USAGE.md` (базова лінія 21 → **15**, може лише зменшуватись) · **експорт барелю на `*ForTests`** (T105; скан — `scripts/barrelSurface.mjs`, має власні тести) · `work/CTO-RESEARCH.md` далі ніж 25 комітів від `HEAD` · **`docs/USAGE.md` згадує `onSeatCollision` і `displacedPositionId`** (T111; `scripts/seatCollisionDocs.mjs`, теж має власні тести). **П'ять** перевірок, дві з власними тестами — разом 13 у `npm run test:scripts` |

...
| `npm run check:docs` | **свіжість доки, перед пушем** (джоба CI `Docs freshness`): биті `.md`-лінки · публічний метод `OrgHierarchyDiagram` поза `docs/USAGE.md` (базова лінія 21 → **15**, може лише зменшуватись) · **експорт барелю на `*ForTests`** (T105; скан — `scripts/barrelSurface.mjs`, має власні тести) · `work/CTO-RESEARCH.md` далі ніж 25 комітів від `HEAD` · **`docs/USAGE.md` згадує `onSeatCollision` і `displacedPositionId`** (T111; `scripts/seatCollisionDocs.mjs`, теж має власні тести). · **`docs/USAGE.md` пояснює, режим ЧОГО називають `getOrgMode`/`onOrgModeChange`** (T113; `scripts/orgModeDocs.mjs`, структурна перевірка — розділ + обидва аксесори в ньому, бо нового ідентифікатора зміна не додає). **Шість** перевірок, три з власними тестами — разом 19 у `npm run test:scripts` |
...
- **Свіжість доки — частково ловиться з 2026-09-03** (`npm run check:docs`, джоба CI
  `Docs freshness`): биті `.md`-лінки, публічний метод поза `docs/USAGE.md`, застарілий базис
  брифінгу. Чого вона **не** ловить — чи твердження досі **правдиве**: `T56` десять днів заявляв
  контур на WASM після того, як його звідти прибрали (`C3`, `H3`). Це видно лише при читанні
  заяви поруч із кодом.

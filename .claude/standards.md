...
| `npm run measure -w @org-hierarchy/sdk` | ⚠️ **НЕ гейт і не в CI** — набір, який хтось мусить запустити руками. Сюди 2026-09-11 винесено все, що читає **годинник** (`*.measure.test.ts`, `exclude` у `rstest.config.ts` знімається змінною `MEASURE=1`) |
...
- ⚠️ **`ai-review.yml` — СПЛЯЧИЙ із 2026-09-08, і це рішення, не поломка.** `ANTHROPIC_API_KEY` у секретах немає й не планується, тож на `pull_request` обидва джоби скіпались би **завжди**. Два вічно скіпнуті чеки гірші за відсутні: рядок, з яким нічого не можна зробити, привчає не читати список. Тригер переведено на `workflow_dispatch` — промти лишились і правильні, повернення коштує чотирьох рядків. **Отже незалежного машинного рев'ю на PR у цьому репо зараз НЕМАЄ**, і планувати на нього не можна.
- **`post-deploy.yml`** — smoke по живому Pages після успішного деплою (`workflow_run`) або вручну.

### 🔴 Що переїхало з сюїти в `measure` — і чого CI через це більше не стереже

**Правило (2026-09-11, T101):** дефолтна сюїта тримає тести, що перевіряють **поведінку** або
**рахують роботу**. Усе, що читає **годинник**, живе в `npm run measure`.

Причина — вимір, а не смак. Під трьома паралельними сюїтами ламаються **обидві** форми
часового тесту: і абсолют (гвардія глибини, стеля 4 000 мс), і частка (`rowTreeLayoutCost` B3,
`< 8`, впав у 2 із 3). Підбирати стелю під кожен окремо — програшна гра: кожен наступний
рівень контенції з'їдає ще один запас. А в валідації **смуга шуму накриває смугу дефекту**:
лінійна на 200k дає 183–271 мс у спокої й 10 846 мс під навантаженням (×40), тоді як навмисно
квадратична відстоїть лише на ×30–60. Стелі між ними не існує.

⚠️ **Ціна, названа прямо:** чотири властивості вартості більше **не мають сторожа в CI** —
вартість row-tree розкладки, вартість фарби магнітних груп, час гвардії глибини й лінійність
`validateOrgHierarchy`. Це свідомий обмін: тест, що червоніє від сусіда, вчить ігнорувати
червоне, і тоді втрачається не одна властивість, а довіра до всього списку.

🔑 **Окремий випадок, який не є порогом і не лікується ним:** `createReactPromoteOverlay` падав
тому, що трансформ, який він перевіряє, **за побудовою тимчасовий** — `sync()` скидає його,
щойно камера осіла. Зі спільним `settleMs = 15` тест вимагав, щоб асерти встигли за 15 мс
стінного часу. Правильна відповідь — **прибрати дедлайн із рівняння** (`settleMs: 10_000`), а не
підняти «із запасом»: чекати там нема на що, бо предмет перевірки — що запис **не** відкладений.
...
⚠️ **Архітектурний факт (оновлено 2026-09-08, T80):** контур має **один** рушій —
TS `render/contour/paintMagneticGroups.ts` плюс виїмки G2/M2. Канвас і SVG-експорт беруть
геометрію **звідти ж**, тож правило «SVG ніколи не малює рушієм, якого не використав канвас»
тепер тримається за побудовою, а не за домовленістю. PNG/PDF знімаються з фреймбуфера.
